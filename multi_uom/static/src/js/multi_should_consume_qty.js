 ...
 */
const MrpMultiShouldConsume = FieldFloat.extend({
    /**
     * @override
     */
    init: function (parent, name, params) {
        ...
        this.multi_should_consume_qty = field_utils.format.float(params.data.multi_should_consume_qty, params.fields.multi_should_consume_qty, this.nodeOptions);
    },
    ...
    _renderEdit: function () {
        if (this.displayShouldConsume) {
            if (!this.$el.text().includes('/')) {
                this.$input = this.$el;
                this._addShouldConsume(this.multi_should_consume_qty, true);
            }
    ...
    _renderReadonly: function () {
        ...
        if (this.displayShouldConsume) {
            this._addShouldConsume(this.multi_should_consume_qty);
        }
    },
});

fieldRegistry.add('mrp_multi_should_consume', MrpMultiShouldConsume);

export default {
    MrpMultiShouldConsume: MrpMultiShouldConsume,
};
