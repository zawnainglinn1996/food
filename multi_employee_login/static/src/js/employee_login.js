odoo.define('multi_employee_login.employee_login', function (require) {
    "use strict";

    var { ViewNotFoundError } = require('@web/webclient/actions/action_service');
    var DataManager = require('web.DataManager');
    var session = require('web.session');
    var rpc = require("web.rpc");
    var { registry } = require('@web/core/registry');
    var action_service = require("@web/webclient/actions/action_service");
    var { browser } = require("@web/core/browser/browser");
    var { makeContext } = require("@web/core/context");
    var { useDebugCategory } = require("@web/core/debug/debug_context");
    var { download } = require("@web/core/network/download");
    var { evaluateExpr } = require("@web/core/py_js/py");
    var { KeepLast } = require("@web/core/utils/concurrency");
    var { useBus, useService } = require("@web/core/utils/hooks");
    var { sprintf } = require("@web/core/utils/strings");
    var { cleanDomFromBootstrap } = require("@web/legacy/utils");
    var { View } = require("@web/views/view");
    var { ActionDialog } = require("@web/webclient/actions/action_dialog");
    var { CallbackRecorder } = require("@web/webclient/actions/action_hook");
    var rpc = require("web.rpc");
    ...
    console.log(CTX_KEY_REGEX,'CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC')
    ...
    function makeActionManager(env) {
        ...
        console.log('START ACTION START ACTION START ACTION START ACTION START ACTION START ACTION------------------')
         ...
         */
        async function _loadAction(actionRequest, context = {}, is_valid=false) {
            ...
            const result = await rpc.query({
                model: 'res.users',
                method: 'get_employee_login',
                args: []
            })
            if (!is_valid && result.is_concurrent_user && !result.emp_id) {
                return _loadAction(result.action_id, context = context, is_valid = true)
            } else {
        ...
        async function _executeClientAction(action, options) {
            ...
            if (clientAction.prototype instanceof Component) {
                if (action.target !== "new") {
                    await action_service.clearUncommittedChanges(env);
                    if (clientAction.target) {
    ...
    action_service.actionService.start = function (env) {
